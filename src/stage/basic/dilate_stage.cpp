#include "dilate_stage.h"


DilateStage::DilateStage()
:StageBase("Dilate") {}

DilateStage::~DilateStage() = default;

void DilateStage::init() {
	createTrackbar("Size", size);
	createTrackbar("Iterations", iterations);
	createTrackbar("Morph Shape", morph_shape);
}

void DilateStage::setData(InitStream& values) {
	values.next().setTrackbarValue(size);
	values.next().setTrackbarValue(iterations);
	values.next().setTrackbarValue(morph_shape);
}

void DilateStage::execute(const StageData& input, StageData& output, const cv::Mat& base_image) {
	cv::Mat output_image;
	...
	cv::dilate(input.get<cv::Mat>(), output_image, kernel, cv::Point(-1, -1), iterations.value);
...
std::string DilateStage::morphShapeToString() const {
